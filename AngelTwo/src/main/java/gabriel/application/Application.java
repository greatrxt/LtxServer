...
import gabriel.utilities.DbUtils;
...
public class Application {
	...
	/**
	 * Get team name for session
	 * @param request
	 * @return
	 */
	public static JSONObject getTeamNameForSession(HttpServletRequest request){
		JSONObject result;
		HttpSession session = request.getSession(false);// false - does not create session if it does not exist
		if(session!=null){
			String teamName = (String) session.getAttribute(TEAM);
			result = new JSONObject();
			result.put(RESULT, teamName);
		} else {
			result = SystemUtils.generateErrorMessage("Invalid Session");	
		}
	 ...
	 * @return
	 */
	public static String getTeam(HttpServletRequest request){
		HttpSession session = request.getSession(false); // false - does not create session if it does not exist
		if(session!=null){
			String teamName = (String) session.getAttribute(TEAM);
			if(teamName!=null){
